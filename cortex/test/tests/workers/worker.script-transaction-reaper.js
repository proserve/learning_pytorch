...
describe('Workers', function() {
  ...
  describe('script-transaction-reaper', function() {
    ...
    it('should run the job successfully', function(callback) {
      ...
      function handler(message, err, result) {
        if (message.name === 'script-transaction-reaper') {
          doneTest(err)
        }
      }
      ...
      modules.db.models.message.updateOne({ name: 'script-transaction-reaper' }, { $set: { trigger: 0 } }, err => {
        if (err) {
          return doneTest(err)
        }
