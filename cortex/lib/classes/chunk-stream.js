const { Transform } = require('stream'),
      ...
      { rBool, rInt, rString, isPlainObject, sortKeys } = require('../utils'),
      { privatesAccessor } = require('../classes/privates')
  ...
  _transform(object, encoding, callback) {
    ...
    if (object && privates.filter(object, privates.idx)) {
      ...
      if (privates.idx !== index) {
        ...
        err = Fault.create('cortex.invalidArgument.unspecified', { reason: `Stream received chunk out of order. Expecting ${privates.idx} but got ${index}` })
