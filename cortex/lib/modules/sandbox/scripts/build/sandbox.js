...
var defined = __webpack_require__(19);
var fails = __webpack_require__(6);
var spaces = __webpack_require__(43);
var space = '[' + spaces + ']';
var non = '\u200b\u0085';
...
module.exports = 1 / $parseFloat(__webpack_require__(43) + '-0') !== -Infinity ? function parseFloat(str) {
  var string = $trim(String(str), 3);
...
$export($export.P + $export.F * __webpack_require__(45)(ENDS_WITH), 'String', {
...
$export($export.P + $export.F * __webpack_require__(45)(INCLUDES), 'String', {
...
$export($export.P + $export.F * __webpack_require__(45)(STARTS_WITH), 'String', {
...
$export($export.P + $export.F * (__webpack_require__(30) != Object || !__webpack_require__(48)(arrayJoin)), 'Array', {
...
__webpack_require__(37)(WEAK_SET, function (get) {
...
var objects = require('objects'),
    clone = require('clone'),
    _require = require('db'),
    BulkOperation = _require.BulkOperation,
    Driver = _require.Driver,
    _require2 = require('inflection'),
    singularize = _require2.singularize,
    pluralize = _require2.pluralize,
...
var Undefined = void 0,
    ...
    faultCodes = void 0,
    Org = void 0,
    faultErrCodeLookup = void 0,
    faultCodeLookup = void 0;
...
if (global.performance === Undefined) {
  var performanceNow = new Date();
  setAndSeal(global, 'performance', {
    now: function now() {
      return Date.now() - performanceNow;
    }
  });
}

var CortexObject = function () {
  function CortexObject(context) {
    _classCallCheck(this, CortexObject);
  ...
  _createClass(CortexObject, [{
    key: 'read',
    value: function read(path, options) {
    ...
    key: 'update',
    value: function update(path, body, options) {
      if (typeof path !== 'string') {
        options = body;
        body = path;
        path = null;
    ...
    key: 'patch',
    value: function patch(ops, options) {
      return objects.patch(this.object, this._id, ops, options);
    }
  }, {
    key: 'push',
    value: function push(path, body, options) {
      if (typeof path !== 'string') {
        options = body;
        body = path;
        path = null;
    ...
    key: 'setOwner',
    value: function setOwner(to) {
      return objects.transfer(this.object, this._id, to);
    }
  }], [{
    key: 'forgetObject',
    value: function forgetObject(name) {
    ...
    key: 'register_object',
    value: function register_object() {
      this.registerObject.apply(this, arguments);
    }
  }, {
    ...
    value: function normalize_context(context) {
    ...
    key: 'setOwner',
    value: function setOwner(id, to) {
      return objects.transfer(this.getObjectName(), id, to);
    }
  }, {
    key: 'getAccessContext',
    value: function getAccessContext(path) {
    ...
    key: 'deleteMany',
    value: function deleteMany(match) {
      return new Driver(this.getObjectName()).deleteMany(match);
    }
  }, {
    key: 'deleteOne',
    value: function deleteOne(match) {
...
Org = createNamedClass('org', CortexObject, function () {
  ...
  this[pObjects] = new Proxy(this[pLoaded], {
    get: function get(target, property) {
      ...
      var name = String(singularize(property)).toLowerCase().trim();

      if (name in target) {
        return target[name];
      }

      var object = runtimes.object.getObjectClass(name);
      if (!object) {
  ...
  this[pScripts] = new Proxy({}, {
    get: function get(target, name) {
      ...
      if (err) throw err;else return require(name);
    }
  });
});
...
Object.assign(CortexObject.as('Account').prototype, {
  hasRole: function hasRole(role) {
    var roleId = ids.couldBeId(role) ? role : consts.roles[role];
...
Object.assign(Error.prototype, {
  ...
  toJSON: function toJSON() {
    ...
    if (this.faults && this.faults.length) {
      json.faults = [];
      this.faults.forEach(function (f) {
  ...
  add: function add(obj) {
    var err = obj instanceof Error ? obj : Error.create(obj);
...
global.Fault = function (_Error) {
  _inherits(Fault, _Error);

  function Fault(code, message, statusCode, name, reason, path, index, resource) {
    _classCallCheck(this, Fault);

    var obj = Fault.normalizeOptions(code, message, statusCode, name, reason, path, [], index, resource);

    var _this = _possibleConstructorReturn(this, (Fault.__proto__ || Object.getPrototypeOf(Fault)).call(this, obj.code));

    _this.faults = [];

    _this.errCode = obj.errCode;
    _this.code = obj.code;
    _this.statusCode = obj.statusCode;
    _this.name = obj.name;
    ...
    if (Array.isArray(obj.faults)) {
      for (var f = 0; f < obj.faults.length; f++) {
        var child = Fault.from(obj.faults[f]);
        if (child) _this.add(child);
      }
    }
  ...
  _createClass(Fault, [{
    ...
    key: 'add',
    value: function add(errOrCode, msg) {
    ...
    key: 'isErrCode',
    value: function isErrCode(code) {
      return typeof code === 'string' && code.indexOf('.') !== -1;
    }
  }, {
    key: 'errCodeToCode',
    value: function errCodeToCode(errCode) {
      if (Fault.isErrCode(errCode)) {
        var _errCode$split = errCode.split('.'),
            _errCode$split2 = _slicedToArray(_errCode$split, 2),
            code = _errCode$split2[1];
    ...
    key: 'codeToErrCode',
    value: function codeToErrCode() {
      var code = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'kError';
      var ns = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'script';
      var detail = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'unspecified';

      return [ns, faultErrCodeLookup[code] || 'error', detail].join('.');
    }
  }, {
    key: 'normalizeOptions',
    value: function normalizeOptions(code, msg, statusCode, name, reason, path, faults, index, resource) {
      ...
      if (isPlainObject(code)) {
        ...
        if (isPlainObject(msg)) {
          obj = _extends({ errCode: errCode, code: code }, msg);
        } else {
          obj = {
            errCode: errCode, code: code, msg: msg, statusCode: statusCode, name: name, reason: reason, path: path, resource: resource, index: index, faults: faults
          };
        }
      }
    ...
    key: 'from',
    value: function from(err, forceError) {
      if (err instanceof Fault) {
        return err;
      }

      var isObject = isPlainObject(err);
      ...
      if (forceError) {
        var errCode = values.rString(isObject ? err.code : 'kError', 'kError');
    ...
    key: 'create',
    value: function create(code, msg, statusCode, name, reason, path, faults, index, resource) {

      return new Fault(Fault.normalizeOptions(code, msg, statusCode, name, reason, path, faults, index, resource));
    }
  }, {
    key: 'validationError',
    value: function validationError(code, msg, statusCode, name, reason, path, index, resource) {
      var valErr = Fault.create('cortex.invalidArgument.validation');
      valErr.index = index;
    ...
    value: function addCode(code) {
      ...
      var message = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'Error';
      var errCode = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 'script.error.unspecified';

      faultCodes[code] = [errCode, name, statusCode, message];
    }
  }, {
    key: 'lookupStatusCode',
    value: function lookupStatusCode(code) {
      var v = faultCodes[code];
      return v ? v[2] : 500;
    }
  }, {
    key: 'lookupMessage',
    value: function lookupMessage(code) {
      var v = faultCodes[code];
      return v ? v[3] : '';
    }
  }, {
    key: 'isFault',
    value: function isFault(fault) {
      return fault && fault instanceof Fault;
    }
  }, {
    key: 'findRegisteredError',
    value: function findRegisteredError(code) {
...
var EventLoop = function () {
  ...
  _createClass(EventLoop, [{
    key: 'getEarliestTimer',
    value: function getEarliestTimer() {
    ...
    key: 'getEarliestWait',
    value: function getEarliestWait() {
      var t = this.getEarliestTimer();
      return t ? t.target - Date.now() : null;
    }
  }, {
    key: 'insertTimer',
    value: function insertTimer(timer) {
      var timers = this.timers,
          n = timers.length;
    ...
    key: 'removeTimerById',
    value: function removeTimerById(timerId) {
      var timers = this.timers,
          n = timers.length;
...
setAndSeal(global, 'main', function (main) {
  var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      _ref$runtime = _ref.runtime,
      runtime = _ref$runtime === undefined ? {} : _ref$runtime;
...
});['Map', 'WeakMap', 'Set', 'WeakSet', 'Symbol', 'regeneratorRuntime'].forEach(function (name) {
...
Buffer.allocUnsafe = function (n) {
  return new Buffer(n);
};[Object, ObjectID, ObjectID.prototype, Buffer, Buffer.prototype].forEach(function (obj) {
  freezeAndSeal(obj);
});
